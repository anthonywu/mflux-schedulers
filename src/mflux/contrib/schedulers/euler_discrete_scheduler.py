...
class EulerDiscreteScheduler(BaseScheduler):
    ...
    def __init__(
        ...
        if trained_betas is not None:
            self.betas = mx.tensor(trained_betas, dtype=mx.float32)
        elif beta_schedule == "linear":
            self.betas = mx.linspace(
                beta_start, beta_end, num_train_timesteps, dtype=mx.float32
            )
        ...
        self.alphas_cumprod = mx.cumprod(self.alphas, dim=0)
    ...
    def set_timesteps(
        ...
        Args:
            ...
            device (`str` or `mx.Device`, *optional*):
        ...
        if sigmas is not None:
            ...
            if timesteps is not None:
                ...
                if self.config.timestep_spacing == "linspace":
                    ...
                    timesteps = mx.array(timesteps)  # .copy()
                    ...
                    timesteps = mx.array(
                        (
                            mx.arange(self.config.num_train_timesteps, 0, -step_ratio)
                        ).round()
                    ).astype(mx.float32)
                    timesteps -= 1
                else:
        ...
        if (
            ...
            self.timesteps = mx.array(timesteps.astype(mx.float32)).to(device=device)
    ...
    def _sigma_to_t(self, sigma, log_sigmas):
        ...
        # get distribution
        # todo: double check work vs. original np newaxis expansion
        dists = log_sigma - mx.expand_dims(log_sigmas, 1)

        # get sigmas range
        low_idx = (
            mx.cumsum((dists >= 0), axis=0)
            .argmax(axis=0)
    ...
    def step(
        ...
        if isinstance(timestep, (int, mx.integer)):
            raise ValueError(
                (
        ...
        if gamma > 0:
            # mx.normal([shape, dtype, loc, scale, key, stream])
            # Generate normally distributed random numbers.
            noise = mx.normal(model_output.shape, model_output.dtype)
            eps = noise * s_noise
    ...
    def get_velocity(
        ...
        if isinstance(timesteps, int) or isinstance(timesteps, mx.integer):
            raise ValueError(
                (
                    "Passing integer indices (e.g. from `enumerate(timesteps)`) as timesteps to"
